...
- документации исходного и целевого решения на PHP/Yii2 (`ias_uch_vnii/docs/**`);
...
- экспорт статистики в Excel (упоминается в `ias_uch_vnii/docs/guides/УЧЕБНЫЙ_README.md`).
...
Текущее состояние (актуализировано):

- связь «заявка ↔ вложение» реализована через таблицу `task_attachments` (нормализовано);
- при скачивании и предпросмотре выполняется проверка прав на заявку (canUserAccessTask);
- для SVG не используется inline; для отдачи файлов устанавливается заголовок `X-Content-Type-Options: nosniff`;
- метаданные и хранение файлов — в соответствии с моделью (web/uploads/tasks/).

При наличии legacy-данных в `tasks.attachments` (JSON) регламент миграции описан в docs/bd/ОПИСАНИЕ_И_ПЛАН_ДЕЙСТВИЙ_БД_ias_vniic.md.
...
По модели данных (см. `docs/bd/МОДЕЛЬ_БД_ias_vniic.md`, `docs/bd/ОСНОВНОЙ_УЧЕТ_И_БД.md`) рассматриваются сущности:
...
Ключевая функциональная связка: заявка Help Desk должна иметь связь с конкретным активом/группой активов, а по активу должна быть доступна история обращений. **Реализовано:** таблица связи `task_equipment`, выбор активов в форме заявки (`equipment_ids`), блок «Связанные заявки» в карточке актива (arm/view).

### 4.4. Контур «ПО и лицензии» (Software/Licenses)
По ТЗ (п. 1.4.6) контур учёта ПО и лицензий входит в состав Системы. **Реализовано:** учёт программного обеспечения, сроков действия лицензий, связь «устройство → установленное ПО»; отчёт «Лицензии: истекающие и просроченные» (software/expiring-report); регламент уведомлений — docs/sistema/УВЕДОМЛЕНИЯ_ЛИЦЕНЗИИ.md.
...
Журнал аудита реализован на AG Grid с серверной фильтрацией (дата, пользователь, тип операции, тип объекта); данные отдаёт эндпоинт `GET /index.php?r=audit/get-grid-data`.
...
- `GET /index.php?r=audit/get-grid-data` — данные журнала аудита для AG Grid (доступ только администратору).
...
По ERD целевой схемы `ias_vniic` (tech_accounting) в рамках Help Desk и учета активов используются сущности:
...
Связь «заявка ↔ вложение» реализована через таблицу `task_attachments`. При наличии legacy-данных в `tasks.attachments` (JSON) регламент миграции описан в docs/bd/ОПИСАНИЕ_И_ПЛАН_ДЕЙСТВИЙ_БД_ias_vniic.md.

### 6.3. Источники данных
В документах переноса выделяются:

- `db/IAS_VNIIC_dump.sql` как актуальный дамп для развёртывания целевой схемы ias_vniic/tech_accounting;
- рабочая БД (при наличии) как источник исторических данных для миграции.
...
- документация по архитектуре, БД, плану реализации и требованиям (TZ.md, `docs/bd/МОДЕЛЬ_БД_ias_vniic.md`, ПЛАН_РЕАЛИЗАЦИИ.md, docs/**).
...
1) **Роль «оператор»**: зафиксирована в docs/sistema/РОЛИ_И_ПРАВА_ДОСТУПА.md (функции, права, ограничения).

2) **Режим работы AG Grid**: зафиксирован в docs/sistema/СПЕЦИФИКАЦИЯ_API.md — клиентская пагинация/фильтрация; при росте объёмов возможна серверная пагинация.

3) **Состав контуров beyond Help Desk**: контуры «Активы», «ПО/лицензии» реализованы; «Закупки», «Плановое обслуживание», «Уведомления» — по ТЗ и дорожной карте при необходимости.

4) **Базовые показатели объемов и производительности**: требуется определить целевые объемы данных и SLA на ключевые операции (при необходимости).

5) **Регламент миграции**: зафиксирован в docs/import_ou/РЕГЛАМЕНТ_МИГРАЦИИ.md; загрузчик — scripts/load_ou_main.py, консольная команда `php yii import-ou`; универсальный импорт — import/universal.
...
- контур учета активов присутствует в предметной области и частично отражен в схемах данных; целевая схема БД — ias_vniic (tech_accounting) (см. docs/ОПИСАНИЕ_И_ПЛАН_ДЕЙСТВИЙ_БД_ias_vniic.md);
