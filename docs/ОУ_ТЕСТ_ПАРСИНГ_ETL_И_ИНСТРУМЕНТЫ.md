...
Документ подготовлен по результатам анализа укороченной версии основного учёта **ОУ_тест.xlsx** и целевой схемы БД (ias_vniic / tech_accounting, DDL в `scripts/create_ias_uch_db_test.sql`).
...
Целевая схема: **tech_accounting** в БД **ias_vniic** (DDL: `scripts/create_ias_uch_db_test.sql`).
