...
Парсинг листа АРМ (ОУ_тест.xlsx / Основной Учет.xlsx), регламент, ETL и примеры. Универсальный импорт (CSV, JSON, XML, XLSX) и регламент миграции.

**Единый источник правил:** регламент парсинга; при расхождении приоритет у регламента.

| Документ | Описание |
|----------|----------|
| [РЕГЛАМЕНТ_ПАРСИНГА.md](РЕГЛАМЕНТ_ПАРСИНГА.md) | Правила парсинга по столбцам, особым значениям, листу Принтеры. |
| [РЕГЛАМЕНТ_МИГРАЦИИ.md](РЕГЛАМЕНТ_МИГРАЦИИ.md) | Регламент миграции/импорта из «Основной Учет.xlsx», порядок вставки, контрольные сверки. |
| [УНИВЕРСАЛЬНЫЙ_ИМПОРТ.md](УНИВЕРСАЛЬНЫЙ_ИМПОРТ.md) | Универсальный импорт CSV, JSON, XML, XLSX: экран import/universal, протокол, валидация. |
...
**Альтернатива (консольная команда приложения):** из каталога приложения (ias_uch_vnii) выполните  
`php yii import-ou`  
или с указанием файла:  
`php yii import-ou "C:\путь\Основной Учет.xlsx"`  
Протокол (количество загруженных записей и ошибки) выводится в консоль скриптом load_ou_main.py.
