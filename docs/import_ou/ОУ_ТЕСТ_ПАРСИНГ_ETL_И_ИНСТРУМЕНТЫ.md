...
**Единый источник правил по столбцам и особым значениям (куда грузить, что не грузить, статусы вроде «Карточка оформлена»):** **[docs/import_ou/РЕГЛАМЕНТ_ПАРСИНГА.md](РЕГЛАМЕНТ_ПАРСИНГА.md)**. Парсер/загрузчик должны следовать регламенту; при расхождении с данным документом приоритет у регламента.
...
**Итог:** парсинг и загрузка **реализуемы без избыточной сложности**. Основные риски: разнообразие форматов дат и неатомарные ячейки; их нужно обрабатывать по зафиксированным правилам (см. п. 4 и `docs/bd/СТРУКТУРА_ЛИСТА_АРМ_ОСНОВНОЙ_УЧЕТ.md`).
...
- **Неатомарные ячейки:** разделители `\n`, запятая. Для полей, уходящих в **equipment** (inventory_number, name), брать первую подстроку или конкатенировать по регламенту. Для характеристик (**part_char_values**) — либо одна запись с полным value_text, либо несколько записей по подстрокам (см. `docs/bd/СТРУКТУРА_ЛИСТА_АРМ_ОСНОВНОЙ_УЧЕТ.md`).
