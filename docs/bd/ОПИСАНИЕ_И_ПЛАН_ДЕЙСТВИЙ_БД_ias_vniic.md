...
Контур «ПО и лицензии» входит в состав Системы и в объём работ по ТЗ. В текущей схеме `ias_vniic` (tech_accounting) таблиц для него **нет**.
...
|-----------|-----------|----------|
| Таблицы software, licenses, software_installs | Обязательно (ТЗ 5.1.12) | Добавить в `scripts/create_ias_vniic.sql` |
...
### 2.4. Нормализация вложений заявок (task_attachments)

Приложение использует нормализованную связь **task_attachments** (task_id, attachment_id) и хранилище **desk_attachments**. В таблице `tasks` может присутствовать поле **attachments_legacy** (JSONB) для совместимости со старыми данными.

**Регламент миграции** (если в legacy-источнике вложения хранились в JSON-поле типа `tasks.attachments`): (1) для каждой записи в tasks с непустым attachments_legacy/attachments распарсить JSON (массив id вложений или путей); (2) для каждого id проверить наличие записи в desk_attachments и при наличии вставить связь в task_attachments (task_id, attachment_id); (3) после проверки целостности поле attachments_legacy можно очистить или оставить как архивный слепок. Одноразовый скрипт миграции выполнять в транзакции с резервной копией БД.
...
- После добавления software, licenses, software_installs и task_comments внести их в описание модели и ER-диаграмму в [МОДЕЛЬ_БД_ias_vniic.md](МОДЕЛЬ_БД_ias_vniic.md).
...
|-----|----------|-----------|
| 1.1 | Добавить в `scripts/create_ias_vniic.sql` таблицы **software**, **licenses**, **software_installs** (в стиле целевой БД: BIGSERIAL, FK на equipment/users). | Контур 5.1.12 обеспечен на уровне схемы. |
...
- Схема **ias_vniic** (tech_accounting) в целом соответствует ТЗ и пригодна для развёрнутого проекта. Для полного соответствия ТЗ необходимо добавить контур учёта ПО и лицензий (таблицы software, licenses, software_installs) и таблицу комментариев к заявкам (task_comments).
