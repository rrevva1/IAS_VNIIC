# Описание и план действий по БД ias_vniic (схема tech_accounting)
...
**Источники:** `TZ.md`, `scripts/create_ias_vniic.sql`, [МОДЕЛЬ_БД_ias_vniic.md](МОДЕЛЬ_БД_ias_vniic.md).
...
Контур «ПО и лицензии» входит в состав Системы и уже реализован в проекте: применена миграция `m260213_120000_add_software_licenses.php`, используются таблицы `software`, `licenses`, `equipment_software`.
...
| **equipment_software** | Связь «актив — установленное ПО» | id, equipment_id (FK), software_id (FK), license_id (FK, nullable), installed_by_user_id (FK→users), installed_at, uninstalled_at, notes |

Дополнительно: поддерживать индексы по software_id, valid_to (для отчётов по истекающим лицензиям), (equipment_id, software_id). Аудит операций с этими сущностями покрывается таблицей `audit_events`.
...
|-----------|-----------|----------|
| Контур software/licenses/equipment_software | Выполнено | Поддерживать синхронизацию DDL, миграций и документации |
...
| Раздел ТЗ | Требование | Состояние в ias_vniic (tech_accounting) |
|-----------|------------|-----------------------------|
...
| 5.1.12 | ПО и лицензии | Реализовано: `software`, `licenses`, `equipment_software`; требуется развитие уведомлений и отчётности по лицензиям. |
...
**Итог:** состояние БД оценивается как **хорошее для запуска**. Обязательным развитием остаётся таблица комментариев к заявкам (`task_comments`) и функциональные доработки, не связанные с базовой структурой контура ПО/лицензий.
...
- Поддерживать в актуальном состоянии описание модели и ER-диаграмму в [МОДЕЛЬ_БД_ias_vniic.md](МОДЕЛЬ_БД_ias_vniic.md), включая уже реализованный контур `software`/`licenses`/`equipment_software` и доработки по `task_comments` (при реализации).
...
|-----|----------|-----------|
| 1.1 | Синхронизировать DDL (`scripts/create_ias_vniic.sql`) с уже применёнными миграциями контура ПО/лицензий: **software**, **licenses**, **equipment_software**. | Контур 5.1.12 зафиксирован в едином эталоне схемы. |
...
| 1.5 | Обновить **МОДЕЛЬ_БД_ias_vniic.md**: новые таблицы, связи, ER. | Актуальная документация. |

### 4.2. Этап 2. Проверка и развёртывание БД

| Шаг | Действие | Результат |
|-----|----------|-----------|
| 2.1 | Создать БД: `CREATE DATABASE ias_vniic WITH ENCODING 'UTF8' TEMPLATE template0;` | Чистая БД. |
| 2.2 | Выполнить полный скрипт `create_ias_vniic.sql` (после внесения изменений этапа 1). | Создание всех объектов и базового наполнения справочников. |
...
| Шаг | Действие | Результат |
|-----|----------|-----------|
| 3.1 | Настроить подключение приложения (PHP или иного) к БД **ias_vniic** (например, через config/db.php или переменные окружения). | Приложение работает с новой схемой. |
| 3.2 | Адаптировать модели и запросы под новую схему: имена таблиц/полей (в т.ч. equipment.inventory_number, status_id, responsible_user_id; task_equipment; task_attachments; при реализации — task_comments; по ПО/лицензиям — software, licenses, equipment_software). | Корректное чтение/запись. |
| 3.3 | При необходимости переноса данных из действующей БД (если она есть): разработать и выполнить скрипты миграции с маппингом полей (arm→equipment, users, roles, tasks→task_attachments из JSON и т.д.). | Исторические данные в целевой БД. |
...
| 4.3 | Развивать контур ПО и лицензий: поддерживать учёт ПО/лицензий/связей «актив — установленное ПО», реализовать уведомления об истечении лицензий (ТЗ 5.1.12.3). | Полное закрытие контура 5.1.12 на уровне эксплуатации. |
...
- Схема **ias_vniic** (tech_accounting) в целом соответствует ТЗ и пригодна для развёрнутого проекта. Контур учёта ПО и лицензий реализован (software, licenses, equipment_software); для полного соответствия ТЗ остаётся таблица комментариев к заявкам (`task_comments`) и эксплуатационные доработки.
